...
func bootstrapAuditToolbeltCommand() string {
	return strings.Join([]string{
		...
		"done",
		"if ! docker compose version >/dev/null 2>&1 && ! docker-compose version >/dev/null 2>&1; then baseline_missing=1; fi",
		"if [ \"$baseline_missing\" -eq 1 ]; then",
		"  if ! command -v apt-get >/dev/null 2>&1; then",
		"    echo \"mnm: the audit VM baseline toolbelt is required but apt-get is unavailable\" >&2",
		"    exit 1",
		"  fi",
		"  apt_install_prefix=\"\"",
		...
		"    baseline_packages=\"$baseline_packages docker-compose-v2\"",
		"  elif apt-cache show docker-compose >/dev/null 2>&1; then",
		"    baseline_packages=\"$baseline_packages docker-compose\"",
		"  fi",
		"  $apt_install_prefix env DEBIAN_FRONTEND=noninteractive apt-get install -y $baseline_packages",
		"fi",
